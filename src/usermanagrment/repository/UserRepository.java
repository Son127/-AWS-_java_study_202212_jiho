...
public class UserRepository {
	...
	public int saveUser(User user) {
		...
		try {
			...
			pool.freeConnection(con, pstmt, rs); // 메모리반환
		}
			...
			while (rs.next()) {
				if (i == 0) {
					user = User.builder().userId(rs.getInt(1)).username(rs.getString(2)).password(rs.getString(3))
							.name(rs.getString(4)).email(rs.getString(5)).build();
				}
				RoleMst roleMst = RoleMst.builder().roleId(rs.getInt(9)).roleName(rs.getNString(10)).build();
